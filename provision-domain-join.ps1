param(
    $domain = 'example.test',
    $domainControllerIp = '10.20.20.10'
)
...
$vagrantManagementAdapter = $adapters | Where-Object { ($_ | Get-NetIPAddress) -notlike "$($domainControllerIp -replace '\.\d+$','.')*" }
$domainControllerAdapter = $adapters | Where-Object { ($_ | Get-NetIPAddress) -like "$($domainControllerIp -replace '\.\d+$','.')*" }


# rename the adapters.
$vagrantManagementAdapter | Rename-NetAdapter -NewName Vagrant
$domainControllerAdapter | Rename-NetAdapter -NewName Domain
