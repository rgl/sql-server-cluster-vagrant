...
(Invoke-Sqlcmd -ServerInstance $env:SQL_SERVER_INSTANCE -Query 'select app_name() as value').value
...
(Invoke-Sqlcmd -ServerInstance "$env:SQL_SERVER_INSTANCE,1433" -Username alice.doe -Password HeyH0Password -Query 'select suser_name() as value').value
